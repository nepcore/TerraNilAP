...
class HillAndDaleLogic : IMissionLogic
{
    ...
    public void Update(GameState state)
    {
        ...
        if (state.progressionState.Tier2ProgressDict.GetValueSafe(Type.RockyScrubland) >= (rockyScrublandTarget * 0.02) && TerraNilAP.Session.Locations.AllMissingLocations.Contains(t2rockyScrubland1))
        {
            TerraNilAP.Session.Locations.CompleteLocationChecks(new long[] {t2rockyScrubland1});
        }

        if (state.progressionState.Tier2ProgressDict.GetValueSafe(Type.RockyScrubland) >= rockyScrublandTarget && TerraNilAP.Session.Locations.AllMissingLocations.Contains(t2rockyScrubland100))
        {
            TerraNilAP.Session.Locations.CompleteLocationChecks(new long[] {t2rockyScrubland100});
        }
